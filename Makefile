LANGUAGE_NAME := tree-sitter-tcsh
HOMEPAGE_URL := https://github.com/j3bit/tree-sitter-tcsh
DESCRIPTION := Tree-sitter grammar for documented tcsh/csh surface syntax.
VERSION := 0.1.0
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/tcsh '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/tcsh
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/tcsh
